...
/** 官方只读词库:右侧预览当前选中的单词 */
let detailWord = $ref<Word | null>(null)
...
async function searchOfficialWord() {
  ...
  // 桌面版无服务器，在线查词不可用，改为在当前词库中查找
  const found = allList.find(v => v.word?.toLowerCase() === word.toLowerCase())
  if (!found) {
    officialWordSnapshot = null
    Toast.warning('当前词库中未找到该单词')
    return
  }
  const normalized = normalizeApiWord(found)
  ...
  await wordFormRef.validate(valid => {
    if (valid) {
      ...
      if (data.id) {
        ...
        // id 用现有最大 id+1,避免删除单词后 length+1 与现存 id 冲突(编辑会命中错误词条)
        data.id = allList.length ? Math.max(...allList.map(v => Number(v.id) || 0)) + 1 : 1
        data.checked = false
    ...
    if (!runtimeStore.editDict.id) {
      ...
      if (!runtimeStore.editDict.words.length && !runtimeStore.editDict.custom && !runtimeStore.editDict.system) {
        ...
        try {
          ...
          // 断网/词库文件缺失:不阻塞页面,loading 照常置位
          Toast.error('加载词库失败,请检查网络')
        }
      }
      loading = false
    }
...
function formClose() {
  if (isEdit) {
    ...
    // 创建入口(主界面"创建个人词典")进入时 editDict 是空的,关闭应返回上一页;
    // 注意:先导航、不切 isAdd,否则会先渲染一帧空的详情页再跳走(闪烁)
    const fromCreateEntry = route.query?.isAdd !== undefined && !runtimeStore.editDict.id
    if (fromCreateEntry) {
...
// ===== 考纲范围学习(ECDICT 词库)=====
// ECDICT 词条带考试标签(zk/gk/cet4/cet6/ky/ielts/toefl/gre),可按考纲范围生成子词库学习
const EXAM_TAGS: { tag: string; label: string }[] = [
  { tag: 'zk', label: '中考' },
  { tag: 'gk', label: '高考' },
  { tag: 'cet4', label: '四级' },
  { tag: 'cet6', label: '六级' },
...
let showTagSelectDialog = $ref(false)
let selectedTag = $ref('all')

// 各考试范围的词数(遍历一次统计,computed 缓存)
const tagCounts = computed<Record<string, number>>(() => {
  const counts: Record<string, number> = {}
  for (const w of runtimeStore.editDict.words as any[]) {
    for (const t of w.tags ?? []) counts[t] = (counts[t] ?? 0) + 1
  }
  return counts
})

// 确认范围:全部 → 原流程;指定范围 → 筛选生成子词库(ECDICT·四级 等)
function confirmTagSelect(): boolean {
  if (selectedTag === 'all') {
    showTagSelectDialog = false
    showPracticeSettingDialog = true
    return true
  }
  const words = (runtimeStore.editDict.words as any[]).filter((w: any) => w.tags?.includes(selectedTag))
  if (!words.length) {
    Toast.warning('该范围内暂无单词')
    return false
  }
  const label = EXAM_TAGS.find(t => t.tag === selectedTag)?.label ?? selectedTag
  const temp = ensureCustomDictCopy(runtimeStore.editDict)
  temp.words = shallowReactive(words)
  temp.length = words.length
  temp.name = `${runtimeStore.editDict.name}·${label}`
  temp.enName = `ecdict-${selectedTag}`
  runtimeStore.editDict = temp
  showTagSelectDialog = false
  showPracticeSettingDialog = true
  return true
...
async function addMyStudyList() {
  ...
  // ECDICT 大词库:先选考试范围(全部/中考/高考/四级/六级/考研/雅思/托福/GRE)
  if (runtimeStore.editDict.enName === 'ecdict') {
    selectedTag = 'all'
...
/** 错词一键重练:以错词本全部单词为队列直接开始练习 */
async function retryWrongWords() {
  const wrong = store.word.wrong.words
  if (!wrong.length) return Toast.warning('错词本是空的')
  // 与 startPractice 一致:先将进行中的练习统计落库,再清缓存
  const cache = await getPracticeWordCacheLocal()
  if (cache) {
    flushStatToStore((cache as any)?.statStoreData)
    await wordPersistence.clear()
  ...
  nav('practice-words/' + store.sdict.id, {}, {
    taskWords: { new: [], review: shuffle(cloneDeep(wrong)) },
    practiceData: null,
    statStoreData: null,
  })
}
...
defineRender(() => {
  return (
    <BasePage>
        ...
        <div className="card mb-0 dict-detail-card flex flex-col">
          <div class="dict-header flex justify-between items-center relative">
            ...
            <div class="dict-actions flex">
              ...
              {runtimeStore.editDict.id === DictId.wordWrong && (
                <BaseButton type="primary" loading={studyLoading || loading} onClick={retryWrongWords}>
                  {'重练错词'}
                </BaseButton>
              )}
          ...
          <div class="flex flex-1 overflow-hidden content-area">
            <div class={`word-list-section ${isMob && isOperate && activeTab !== 'list' ? 'mobile-hidden' : ''}`}>
              <BaseTable
                ...
                readonly={!editable}
                readonlyTip="官方资源不可编辑，请先创建副本"
              >
                {val => (
                  <WordItem
                    showTransPop={false}
                    onClick={() => (editable ? editWord(val.item) : (detailWord = val.item))}
                    index={val.index}
                    active={!editable && detailWord?.id === val.item.id}
                      ...
                      suffix: () => (
                        <div class="flex flex-col">
                          <BaseIcon
                            class="option-icon"
                            disabled={!editable}
                            title={runtimeStore.editDict.custom ? '编辑' : '官方资源不可编辑，请先创建副本'}
                          ...
                          {editable ? (
                            ...
                            <BaseIcon class="option-icon" disabled={true} title="官方资源不可编辑，请先创建副本">
                              <DeleteIcon />
                            </BaseIcon>
                          )}
                        </div>
            ...
            {isOperate ? (
              ...
              /* 官方只读词库:右侧展示单词详情(点击左侧单词预览) */
              <div class="edit-section flex-1 flex flex-col min-w-0">
                <div class="common-title">{'单词详情' }}</div>
                {detailWord ? (
                  <div class="flex-1 min-h-0">
                    <WordDetail word={detailWord} />
                  </div>
                ) : (
                  <div class="center flex-1 text-gray">{'点击左侧单词查看详情' }}</div>
                )}
                <div class="text-center text-sm text-gray py-2">{'官方词库为只读，想修改请点右上角“创建副本”' }}</div>
              </div>
            ) : null}
          </div>
        </div>
      ...
      {/* 考纲范围选择(ECDICT 词库) */}
      <Dialog
        modelValue={showTagSelectDialog}
        onUpdate:modelValue={val => (showTagSelectDialog = val)}
        title="选择学习范围"
        padding
        footer
        onConfirm={confirmTagSelect}
      >
        <div class="exam-tag-list">
          <div
            class={['exam-tag-item', { active: selectedTag === 'all' }]}
            onClick={() => (selectedTag = 'all')}
          >
            <span>{'全部单词' }}</span>
            <span class="count">{runtimeStore.editDict.words.length} {'单词'}</span>
          </div>
          {EXAM_TAGS.map(t => (
            <div
              class={['exam-tag-item', { active: selectedTag === t.tag }]}
              onClick={() => (selectedTag = t.tag)}
            >
              <span>{t.label}</span>
              <span class="count">{tagCounts[t.tag] ?? 0} {'单词'}</span>
            </div>
          ))}
        </div>
        <div class="exam-tag-tip">{'按范围学习时,生成对应子词库(如「ECDICT·四级」),只学习该考纲内单词'}</div>
      </Dialog>
    </BasePage>
  )
})
...
.exam-tag-list {
  ...
  .exam-tag-item {
    ...
    &.active {
      background: var(--btn-primary, var(--color-select-bg));
      color: white;
